...
[^1]: Almost the same outcome. Multinomial regression is more efficient. Our errors will be slightly different to those we would obtain if we fit multiple binary regressions. 
...
Let's explore this by looking at the level of support university students had for the Iraq war. 

First, let's load and clean our data:

```{r}
student_survey <- rio::import("/Users/harrietgoers/Downloads/student vote GVPT 729a.dta") |> 
  select(warsup, female, pid) |> 
  drop_na()
...
And the crosstab to see if anything jumps out at us:

```{r}
tabyl(student_survey, warsup, pid, female)
```

Next, let's fit our model: 

```{r}
m1 <- multinom(warsup ~ 1 | female + pid, data = student_survey)
